class Portfolio < ApplicationRecord
    ...
    def rebalance
        ...
        # esta funcion recorre los stocks del portafolio, suma su cantidad total en precio
        ...
        # en caso de que los aims no sumen 100 no es posible hacer el rebalanceo, se debe retornar un error
        # tener en consideracion que los valores estan siendo redondeados a 1 decimal
        ...
        # suma de los porcentajes de los aims
        total_percentage = portfolio_stock_aims.sum(:percentage)
        # si no suman 100, no se puede hacer el rebalanceo
        if total_percentage != 100
            return [], "No se puede hacer el rebalanceo, los porcentajes de los objetivos no suman 100."
        end
        ...
        portfolio_stocks.each do |portfolio_stock|
            ...
            unless portfolio_stock_aims.any? { |aim| aim.stock_id == portfolio_stock.stock_id }
                ...
                if amount_to_sell > 0
                    updates.push(StockAction.new("sell", portfolio_stock.stock, amount_to_sell))
                end
            end
        ...
        return updates, "Rebalanceo completado con #{updates.size} acciones a actualizar."
    end
end
